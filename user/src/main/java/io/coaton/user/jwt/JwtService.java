package io.coaton.user.jwt;

import io.coaton.user.configuration.MyUserDetails;
import io.coaton.user.configuration.MyUserDetailsService;
...
public class JwtService {
    ...
    // We need to store these three compoenents in a format that allows us to deal
    // with them properly, and that is why we are using HashMap()
    // A HashMap allows storing items in "key/value" pairs, and you can access them
    // by an index of another type (e.g. a String).
    // After that, we are passing the claims (HashMap()) and the user name to
    // another function to create the jwt token.
    ...
    private String createToken(Map<String, Object> claims, String userName) {
        // This is not required as user authorities are verified in
        // SecurityConfig/securityFilterChain()
        // I am adding the user authorities/roles to the claims of the jwt token
        // Just an example for some of you that might need to add some user data (e.g.,
        // isEmployee, isAdmin) to the claims of the jwt token
        ...
        // After, extracting the authorities/roles and removing the "[" "]", e are
        // adding the authorities/roles to the claim
        ...
        return Jwts.builder() // Adds all given name/value pairs to the JSON Claims in the payload.
                ...
                // The subject of our JWT is the user in our case, and here we are using her/his
                // name
                .setSubject(userName)
                // .addClaims(roles)
                .setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60)) // when setting an expiration
                                                                                      // "Date", we use milliseconds,
                                                                                      // the easiet way to set it is
                                                                                      // using the following format
                                                                                      // 1000(1000 milliseconds =
                                                                                      // second)*60 (1 second * 60 = a
                                                                                      // minutes) *60 (1 minute * 60 = 1
                                                                                      // hour) -> 1000*60*60 = 1 hour
                // .signWith(Key key, SignatureAlgorithm alg) signs the constructed JWT with the
                // specified key using the specified algorithm, producing a JWS.
                // For the key, we are using a function to created, and for the
                // SignatureAlgorithm we are using the "HS256" hashing algorithm.
                // .compact() is the final step, and it builds the JWT and serializes it to a
                // compact, URL-safe string according to the JWT Compact Serialization rules.
                .signWith(signingKey(), SignatureAlgorithm.HS256).compact();
    }

    // For generating a secret key, You can use
    ...
    private Key signingKey() {
        // Creating a byte array for our key by decoding our SECRET using BASE64
        // encoding scheme
        byte[] keyDecoder = Decoders.BASE64.decode(SECRET);
        // Keys.hmacShaKeyFor(bytes) converts the byte array into a valid Key using the
